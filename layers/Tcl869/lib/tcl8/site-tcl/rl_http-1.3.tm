		...
		{*}$start_thread {
			...
			while 1 {
				...
				foreach {key chaninfo} $to_close {
					...
					try {
						ns_log notice "Closing expired channel $chan to $key"
						thread::attach $chan
						close $chan
					} on error {errmsg options} {
						# TODO: what?
						ns_log notice "Error retiring expired parked channel $chan to $key: [dict get $options -errorinfo]"
					}
				}
			}
		}
...
::gc_class create ::rl_http {
	...
	constructor {a_method url args} { #<<<
		...
		if {[dict exists $response headers connection] && "close" in [dict get $response headers connection]} {
			...
			my _keepalive_park $sock $u(scheme) $u(host) $u(port) 120
			unset sock
		}
	}
	...
	method _keepalive_connect {scheme host port} { #<<<
		#ns_log notice "[self] _keepalive_connect $scheme $host $port"
		...
		#ns_log notice "Looking for parked connection $key: [tsv::array get rl_http_keepalive_chans]"
		while {[set chaninfo [tsv::lpop rl_http_keepalive_chans $key 0]] ne ""} {
			lassign $chaninfo chan expiry
			#ns_log notice "[self] reusing $chan for $scheme://$host:$port"
			try {
				thread::attach $chan
				...
				chan read $chan
				if {[chan eof $chan]} {
					ns_log notice "parked chan collapsed: $chan"
					chan close $chan
					continue
				}
				# Check if the remote closed on us >>>
			} on ok {} {
				#chan push $chan tapchan
				return $chan
			} on error {errmsg options} {
				ns_log notice "Error attaching to parked chan \"$chan\": [dict get $options -errorinfo]"
			}
		}
		ns_log notice "Falling back on opening new connection $scheme://$host:$port"
		switch -- $scheme {
	...
	method _keepalive_park {chan scheme host port timeout} { #<<<
		#ns_log notice "Parking $scheme://$host:$port"
	...
	method _readable_headers {} { #<<<
		while 1 {
			...
			if {[chan blocked $sock]} {
				#ns_log notice "chan blocked ($line)"
				return
			}
	...
	method _read_chunk_data length { #<<<
		...
		#ns_log notice "Setting chunk buffersize to [expr {min(1000000, $expecting)}] (waiting for $length bytes plus crlf)"
